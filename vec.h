 ...
 public:
  Vec(const T v[]){
	for(indexType i=0; i<dim; i++){
	  ...
	  template<indexType dim2, class U, class p >
	  Vec(Vec<dim2,U,p> v){
		for(indexType i=0; i<min(dim,dim2); i++){
			x[i]=v[i];
			}
		};
	...
	inline
	  Vec<_dim, T, policy> Vec<_dim, T, policy>::normalized()const{
		return move(Vec<_dim, T, policy> (*this).normalize());
		}
	...
	Vec<3, T, policy> cross(const Vec<_dim, T, policy> u, const Vec<_dim, T, policy> v){
		...
		return move(Vec<3, T, policy> (policy::mul(u(1),v(2))-policy::mul(u(2),v(1)),policy::mul(u(2),v(0))-policy::mul(u(0),v(2)),policy::mul(u(0),v(1))-policy::mul(u(1),v(0))));
	}
